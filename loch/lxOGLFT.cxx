...
namespace OGLFT {
  ...
  void Face::setForegroundColor ( const wxColour foreground_rgba )
  {
    GLfloat foreground_color[4];
    foreground_color[R] = foreground_rgba.Red() / 255.;
    foreground_color[G] = foreground_rgba.Green() / 255.;
    foreground_color[B] = foreground_rgba.Blue() / 255.;
    foreground_color[A] = 1.0;
  ...
  void Face::setBackgroundColor ( const wxColour background_rgba )
  {
    GLfloat background_color[4];
    background_color[R] = background_rgba.Red() / 255.;
    background_color[G] = background_rgba.Green() / 255.;
    background_color[B] = background_rgba.Blue() / 255.;
...
#ifndef OGLFT_NO_WX
  BBox Face::measure ( const wxString& s )
  {
    BBox bbox;

    if ( s.length() > 0 ) {

      bbox = measure( (const unsigned char) s.GetChar( 0 ) );

      for ( unsigned int i = 1; i < s.length(); i++ ) {

	BBox char_bbox = measure( (const unsigned char) s.GetChar( i ) );
  ...
  BBox Face::measure ( const wxString& format, double number )
  {
    return measure( format_number( format, number ) );
  }

  BBox Face::measureRaw ( const wxString& s )
  {
    BBox bbox;
    ...
    for ( unsigned int i = 0; i < s.length(); i++ ) {
      ...
      for ( f = 0; f < faces_.size(); f++ ) {
	glyph_index = FT_Get_Char_Index( faces_[f].face_, s.GetChar( i ) );
	if ( glyph_index != 0 ) break;
      }
  ...
  BBox Face::measure_nominal ( const wxString& s )
  {
    if ( string_rotation_ == 0. )
      return measure( s );
  ...
  wxString Face::format_number ( const wxString& format, double number )
  {
    return wxString::Format(format, number);
	}
  ...
  GLuint Face::compile ( const wxString& s )
  {
    // First, make sure all the characters in the string are themselves
    // in display lists
    for ( unsigned int i = 0; i < s.length(); i++ ) {
      compile( (const unsigned char) s.GetChar( i ) );
    }
  ...
  GLuint Face::compile ( const wxChar c )
  {
    // See if we've done it already
  ...
  void Face::draw ( const wxString& s )
  {
    DLCI character_display_list = character_display_lists_.begin();
	...
	character_display_list++;
      ...
      draw( (const unsigned char) s.GetChar( i ) );
    }
  }
#endif /* OGLFT_NO_WX */
  ...
  void Face::draw ( GLfloat x, GLfloat y, wxChar c )
  {
    glTranslatef( x, y, 0. );
  ...
  void Face::draw ( GLfloat x, GLfloat y, GLfloat z, wxChar c )
  {
    glTranslatef( x, y, z );
  ...
  void Face::draw ( GLfloat x, GLfloat y, const wxString& s )
  {
    if ( !advance_ )
      glPushMatrix();
  ...
  void Face::draw ( GLfloat x, GLfloat y, GLfloat z, const wxString& s )
  {
    if ( !advance_ )
      glPushMatrix();
  ...
  void Face::draw ( GLfloat x, GLfloat y, const wxString& format, double number )
  {
  ...
  void Face::draw ( GLfloat x, GLfloat y, GLfloat z, const wxString& format,
		    double number )
  {
...
#ifdef _UNICODE
#ifndef OGLFT_NO_WX
  BBox Raster::measure ( const wxChar c )
  {
    BBox bbox;
    // For starters, just get the unscaled glyph bounding box
    unsigned int f;
...
#ifdef _UNICODE
#ifndef OGLFT_NO_WX
  BBox Polygonal::measure ( const wxChar c )
  {
    BBox bbox;
    // For starters, just get the unscaled glyph bounding box
    unsigned int f;
  ...
  BBox Texture::measure ( const wxChar c )
  {
    BBox bbox;
    // For starters, just get the unscaled glyph bounding box
    unsigned int f;
